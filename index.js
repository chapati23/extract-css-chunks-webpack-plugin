...
ExtractTextPluginCompilation.prototype.addResultToChunk = function(
  ...
  result.forEach(function(item) {
    ...
    var module = this.addModule.call(
      this,
      item[0] + (c || ''),
      originalModule,
      item[1],
...
ExtractTextPlugin.prototype.renderExtractedChunk = function(chunk) {
  ...
  forEachChunkModule(chunk, function(module) {
    ...
    source.add(
      that.applyAdditionalInformation(
        moduleSource,
...
function ExtractTextPlugin(options) {
  ...
  if (arguments.length > 1) {
    throw new Error(
      'Breaking change: ExtractTextPlugin now only takes a single argument. Either an options ' +
...
ExtractTextPlugin.prototype.applyAdditionalInformation = function(
  source,
  info
) {
  if (info) {
...
ExtractTextPlugin.prototype.extract = function(options) {
  if (arguments.length > 1) {
    throw new Error(
      'Breaking change: extract now only takes a single argument. Either an options ' +
...
ExtractTextPlugin.prototype.apply = function(compiler) {
  ...
  compiler.plugin(
    ...
    function(compilation) {
      ...
      compilation.plugin(
        'optimize-tree',
        function(chunks, modules, callback) {
          ...
          extractedChunks.forEach(function(extractedChunk) {
            forEachChunkModule(extractedChunk, function(module) {
              ...
              var file = getFile(
                ...
                filename,
                module,
                extractedChunk.originalChunk
              )
              originalModule._source._value = originalModule._source._value.replace(
                '%%extracted-file%%',
                file
              )
            })
...
function isChunk(chunk, error) {
  if (!(chunk instanceof Chunk)) {
    throw new Error(
      typeof error === 'string' ? error : 'chunk is not an instance of Chunk'
    )
  }
